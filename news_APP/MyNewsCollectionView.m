...
@end
@implementation MyNewsCollectionView
/**
...
+(instancetype)titleCollectionViewWithTitles:(NSArray *)titles{
    ...
    //预估的itemSize
    flowLayout.estimatedItemSize = CGSizeMake(70, 33);
    //sectionInsets
    flowLayout.sectionInset = UIEdgeInsetsMake(0, 10, 0, 10);
    MyNewsCollectionView *newsCollectionView = [[MyNewsCollectionView alloc] initWithFrame:CGRectZero collectionViewLayout:flowLayout];
    ...
    //注册cell
    [newsCollectionView registerNib:[UINib nibWithNibName:NSStringFromClass([MyNewsCollectionViewCell class]) bundle:nil] forCellWithReuseIdentifier:@"item"];
    
    newsCollectionView.currenIndex = 0;
...
-(void)setCurrentIndex:(NSUInteger)currenIndex{
    //1.把之前选中的菜单标题置为黑色(_currentIndex)
    MyNewsCollectionViewCell *cell = (MyNewsCollectionViewCell *)[self cellForItemAtIndexPath:[NSIndexPath indexPathForItem:_currenIndex inSection:0]];
    cell.label.textColor = [UIColor blackColor];
    //2.把将要选中的菜单标题置为红色(currentIndex)
    MyNewsCollectionViewCell *willSelctedCell = (MyNewsCollectionViewCell *)[self cellForItemAtIndexPath:[NSIndexPath indexPathForItem:currenIndex inSection:0]];
